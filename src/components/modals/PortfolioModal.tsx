...
import { isPdfDocumentUrl, normalizePublicAssetUrl } from '../../utils/pdf-utils';
...
export const PortfolioModal = ({ project, onClose, lang }: PortfolioModalProps) => {
    const getInitialPdf = (selectedProject: PortfolioProject) =>
        selectedProject.presentationUrl
            ? normalizePublicAssetUrl(selectedProject.presentationUrl)
            : selectedProject.details?.documents?.[0]?.url
    ...
    useEffect(() => {
        ...
        setCurrentPdf(getInitialPdf(project));
        setIsClosing(false);
    ...
    const currentDocumentUrl = currentPdf ? normalizePublicAssetUrl(currentPdf) : null;

    // Basic check for PDF files (local or remote)
    const isPdf = currentDocumentUrl ? isPdfDocumentUrl(currentDocumentUrl) : false;
        ...
        <div ref={modalRef} className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 md:p-6 transition-opacity duration-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`} role="dialog" aria-modal="true" aria-label={project.title}>
            ...
            <div className={`bg-[var(--card-bg)] backdrop-blur-3xl border border-[var(--card-border)] w-full max-w-6xl max-h-[90vh] rounded-[2rem] shadow-2xl overflow-hidden relative flex flex-col landscape:flex-row md:flex-row transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-6 scale-[0.985]'}`}>
                ...
                <div className="w-full landscape:w-1/2 md:w-1/2 h-[40vh] landscape:h-auto md:h-auto bg-black relative flex flex-col">
                    ...
                    {images.length > 1 && (<div className="absolute bottom-4 left-0 right-0 flex justify-center gap-2 px-4 z-20">{images.map((img: string, idx: number) => (<button key={img} onClick={() => setActiveScreenshot(idx)} className={`w-12 h-12 rounded-lg overflow-hidden border-2 transition-all ${activeScreenshot === idx ? `scale-110` : 'border-white/30 opacity-70 hover:opacity-100'}`} style={activeScreenshot === idx ? { borderColor: DYNAMIC_COLORS.raw.light.primary } : {}} aria-label={`Ver screenshot ${idx + 1} de ${project.title}`} aria-pressed={activeScreenshot === idx}><img src={img} alt="" aria-hidden="true" className="w-full h-full object-cover" /></button>))}</div>)}
                </div>
                <div className="w-full landscape:w-1/2 md:w-1/2 p-6 md:p-10 overflow-y-auto custom-scrollbar flex flex-col flex-1 min-h-0">
                    <div className="mb-6"><span className="font-bold uppercase tracking-widest text-xs mb-2 block" style={{ color: DYNAMIC_COLORS.raw.light.primary }}>{t.caseStudy}</span><h2 className="text-3xl md:text-4xl font-bold text-[var(--text-primary)] mb-2 leading-tight">{project.title}</h2><div className="flex flex-wrap gap-2 mt-3">{project.tech.split(',').map((t: string) => (<span key={t.trim()} className="px-3 py-1 rounded-full bg-[var(--input-bg)] border border-[var(--card-border)] text-[10px] font-bold uppercase tracking-wider text-[var(--text-secondary)]">{t.trim()}</span>))}</div></div>
                    <div className="space-y-8">
                        ...
                        {currentDocumentUrl && (
                            <div className="mb-4">
                                ...
                                <div className="w-full h-[300px] md:h-[450px] rounded-xl overflow-hidden border border-[var(--card-border)] shadow-sm bg-[var(--card-bg)] relative">
                                    {isPdf ? (
                                        <PDFViewer url={currentDocumentUrl} showZoomControls={false} />
                                    ) : (
                                        <iframe src={pdfEmbedSrc!} title="Project Presentation" className="w-full h-full">
                                            <div className="flex flex-col items-center justify-center h-full">
                                                ...
                                                <a href={currentDocumentUrl} target="_blank" rel="noreferrer" className="font-bold" style={{ color: DYNAMIC_COLORS.raw.light.primary }}>{t.downloadPdf}</a>
                                            </div>
                                        </iframe>
                                    )}
                                </div>
                                <div className="mt-2 text-right">
                                    <a href={currentDocumentUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-bold hover:underline flex items-center justify-end gap-1" style={{ color: DYNAMIC_COLORS.raw.light.primary }}>
                        ...
                        {project.details?.documents && (<div><h3 className="text-lg font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2"><Icons.Book className="w-5 h-5 text-indigo-500" /> {t.documentation}</h3><ul className="grid grid-cols-1 md:grid-cols-2 gap-3">{project.details.documents.map((doc: { label: string; url: string }) => { const normalizedDocUrl = normalizePublicAssetUrl(doc.url); const isActive = currentDocumentUrl === normalizedDocUrl; return (<li key={doc.url || doc.label}><button onClick={() => setCurrentPdf(normalizedDocUrl)} className={`w-full flex items-center gap-3 p-3 rounded-xl border transition-all group h-full text-left ${isActive ? 'bg-indigo-500/10 border-indigo-500 shadow-md ring-1 ring-indigo-500/30' : 'bg-[var(--input-bg)] border-[var(--card-border)] hover:bg-[var(--glass-glow)]'}`} aria-pressed={isActive}><div className={`p-2 rounded-lg transition-colors ${isActive ? 'bg-indigo-500 text-white' : 'bg-indigo-500/10 text-indigo-500 group-hover:bg-indigo-500 group-hover:text-white'}`}><Icons.Book className="w-4 h-4" /></div><span className={`text-sm font-medium leading-tight ${isActive ? 'text-[var(--text-primary)] font-bold' : 'text-[var(--text-primary)]'}`}>{doc.label}</span>{isActive && <div className="ml-auto w-2 h-2 rounded-full bg-indigo-500"></div>}</button></li>); })}</ul></div>)}
                        ...
                        {project.details?.currentFeatures && (<div><h3 className="text-lg font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2"><Icons.Layers className="w-5 h-5 text-blue-500" /> {t.features}</h3><ul className="grid grid-cols-1 gap-2">{project.details.currentFeatures.map((f: string) => (<li key={f} className="flex items-start gap-3 text-sm text-[var(--text-secondary)] bg-[var(--input-bg)] p-3 rounded-xl border border-[var(--card-border)]"><Icons.CheckCircle className="w-4 h-4 mt-0.5 flex-shrink-0" style={{ color: DYNAMIC_COLORS.raw.light.primary }} /><span>{f}</span></li>))}</ul></div>)}
                        {project.details?.techStack && (<div><h3 className="text-lg font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2"><Icons.Code className="w-5 h-5 text-orange-500" /> {t.techStack}</h3><ul className="grid grid-cols-1 gap-2">{project.details.techStack.map((f: string) => (<li key={f} className="flex items-start gap-3 text-sm text-[var(--text-secondary)] font-mono opacity-80"><span className="font-bold" style={{ color: DYNAMIC_COLORS.raw.light.primary }}>{'>'}</span><span>{f}</span></li>))}</ul></div>)}
                        {project.details?.upcomingFeatures && (<div><h3 className="text-lg font-bold text-[var(--text-primary)] mb-3 flex items-center gap-2"><Icons.Rocket className="w-5 h-5 text-purple-500" /> {t.roadmap}</h3><ul className="space-y-2">{project.details.upcomingFeatures.map((f: string) => (<li key={f} className="flex items-start gap-3 text-sm text-[var(--text-secondary)] opacity-80"><div className="w-1.5 h-1.5 rounded-full bg-purple-500 mt-2 flex-shrink-0"></div><span>{f}</span></li>))}</ul></div>)}
                        <div className="pt-4 border-t border-[var(--card-border)] flex flex-col gap-3">{project.repoUrl && (<a href={project.repoUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm font-bold transition-colors group hover:![color:var(--hover-color)]" style={{ color: 'var(--text-primary)', '--hover-color': DYNAMIC_COLORS.raw.light.primary } as React.CSSProperties}><Icons.GitHub className="w-5 h-5" />{t.viewRepo}<Icons.ExternalLink className="w-4 h-4 opacity-50 group-hover:opacity-100 transition-opacity" /></a>)}</div>
                    </div>
                </div>
            </div>
        </div>,
